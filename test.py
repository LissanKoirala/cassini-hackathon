...
BAND_NAME = "CLOUD_OPTICAL_THICKNESS"
MIN_VALUE = 0.0
MAX_VALUE = 250.0
...
def read_single_band_tiff(content: bytes, label: str) -> np.ndarray:
    """Decode a single-band GeoTIFF response into a 2D numpy array."""
    try:
...
def read_multi_band_tiff(content: bytes, label: str) -> np.ndarray:
    """Decode a multi-band GeoTIFF response into a 3D numpy array."""
    try:
        array = tifffile.imread(BytesIO(content))
    except (tifffile.TiffFileError, ValueError) as exc:
        raise RuntimeError(
...
# Request the scene in tiles to preserve maximum resolution over the full bbox.
...
thickness_array = index_mosaic
mask_array = mask_mosaic

if thickness_array.shape != mask_array.shape:
    raise RuntimeError(
        f"Shape mismatch between thickness {thickness_array.shape} and dataMask {mask_array.shape}."
    )

mask_norm = np.where(mask_array > 0, 1.0, 0.0)
thickness_masked = thickness_array.astype(np.float32)
thickness_masked[mask_norm < 0.5] = np.nan

cloud_band = thickness_masked.tolist()
mask_band = mask_norm.tolist()

band_key = BAND_NAME.lower()
result = {
    "bbox": FULL_BBOX,
    "width": int(thickness_masked.shape[1]),
    "height": int(thickness_masked.shape[0]),
    band_key: cloud_band,
    "data_mask": mask_band
}
...
print(f"Saved {visual_path} and {BAND_NAME}.json")
